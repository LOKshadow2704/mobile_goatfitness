    ...
    const postPaymentDetails = async () => {
      ...
      try {
        ...
        const response = await axios.post(
          `${Constants.expoConfig?.extra?.API_URL}/order/payment`,
          { orderCode: orderCode },
          {
            headers: {
              PHPSESSID: phpSessId,
  ...
  return (
    <View style={styles.container}>
      ...
      {loading ? (
        ...
        <Text style={[styles.title, { color: statusColor }]}>{paymentStatus}</Text>
      )}
      <Text style={styles.description}>Cảm ơn bạn đã sử dụng dịch vụ</Text>
      {error && <Text style={styles.errorText}>{error}</Text>}
      <TouchableOpacity style={styles.button} onPress={() => route.push("/Home")}>
